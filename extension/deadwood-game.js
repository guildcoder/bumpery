(() => {
  ...
  const renderer=new DeadwoodRenderer($('table'));
  ...
  const show=new DeadwoodShow(cabinet);
  const modeGuide=document.createElement('p');modeGuide.textContent='Shoot the lit saloon pocket to lock the ball for a showdown. Release both flippers. After 3, 2, 1, wait through the unpredictable pause for FIRE, then tap either flipper. Drawing early or too late ends the showdown. Each win pays a bonus and brings a faster opponent. Light three lanterns or clear all targets to relight the saloon. The table freezes during a showdown. Three quick nudges tilt.';$('guide').append(modeGuide);
  ...
  const game=new Deadwood.Game(event=>{
    renderer.effect(event);
    switch(event.type){
      case 'loaded':message(touch?'Hold Launch, then release.':'Hold Space. Release to ride into town.');break;
      case 'save':message('A second chance. Ball saved—launch again.');tone(330,.4);break;
      case 'launch':message('Shoot the lit saloon pocket.');tone(140,.25,'triangle');break;
      case 'chaseStart':clearInputs();setDrawing(true);message('Wait for FIRE, then tap either flipper.');tone(160,.5,'triangle');break;
      case 'drawCue':message('FIRE!');tone(880,.12,'triangle');break;
      case 'drawWin':message('You were quicker. Another opponent approaches.');tone(660,.25);break;
      case 'chaseHit':tone(70,.2,'sawtooth');break;
      case 'chaseCrash':message('Showdown over. Returning to the table.');tone(60,.6,'triangle');break;
      case 'chaseEnd':clearInputs();setDrawing(false);message('Showdown complete. +'+format(event.bonus)+' bonus.');show.announce('SHOWDOWN BONUS +'+format(event.bonus),'','combo',game.time,4);tone(660,.3);break;
      case 'gear':message('Rank '+game.gear+'! '+game.multiplier+'× scoring. Saloon open.');show.announce('RANK '+game.gear+' · SALOON LIT','','combo',game.time,3);tone(740,.3);break;
      case 'lockReady':message('Saloon open. Shoot the illuminated pocket.');tone(880,.2);break;
      ...
      case 'superCombo':message('Gunslinger combo! Six different shots.');show.announce('GUNSLINGER COMBO',`+${format(2500*game.multiplier)} BONUS`,'hyper',game.time,3);tone(1047,.4);break;
      ...
      case 'nudge':message(event.danger?'Easy, partner. One more nudge risks tilt.':'A little frontier luck.');tone(85,.12,'triangle');break;
      ...
      case 'over':updateBest();showOverlay('RUN COMPLETE','Sunset on your run.',`Final score: ${format(game.score)}<br>Personal best: ${format(best)}`,'Play again ↗');shareButton.hidden=false;finishRankedRun();message('Run complete. One more ride?');break;
    }
  });
  function setDrawing(on){for(const id of ['left','right']){$(id).querySelector('span').textContent=on?'FIRE':'FLIP';$(id).setAttribute('aria-label',(id==='left'?'Left':'Right')+(on?' drawing':' flipper'));}$('launch').disabled=on;nudgeButton.disabled=on;}
  ...
  function setPause(value){if(game.state==='ready'||game.state==='over')return;if(!value)closeMenu();paused=value;clearInputs();accumulator=0;$('pause').textContent=paused?'▶ Resume':'Ⅱ Pause';shareButton.hidden=true;if(paused){updateBest();showOverlay('TAKE A BREATH','Rest your boots.','Your game is paused.<br>Resume when you’re ready.','Resume game ↗');}else{$('overlay').hidden=true;unlockAudio();}}
  function start(){closeMenu();unlockAudio();if(paused){setPause(false);$('start').blur();return;}runGeneration++;runPromise=leaderboard.begin().catch(()=>null);game.start();clearInputs();setDrawing(false);show.reset();paused=false;accumulator=0;renderer.effects=[];renderer.trail=[];shareButton.hidden=true;$('overlay').hidden=true;$('pause').textContent='Ⅱ Pause';$('start').blur();}
  ...
  function frame(now){
    ...
    if(!paused){accumulator+=dt;while(accumulator>=Deadwood.STEP){game.step();accumulator-=Deadwood.STEP;}}
